...
export default function Page ({params}) {
  ...
  useEffect(() => {
    async function getOrderProducts() {
      ...
      const localOrderProducts = JSON.parse(localOrderData.orderProducts)
      localOrderProducts.forEach(async product => {
        try {
          const productFetch = await fetchProduct(userToken, product.productId)
          productsFetched.push(productFetch) 
        } catch (error) {
          console.error(error)
  ...
  return (
    <AuthPageManager>
      ...
      <main className="w-full min-h-[60%] pt-[100px]">
        ...
        <div className="w-[90%] max-w-[500px] mb-16 py-4 px-6 mx-auto rounded-xl bg-white shadow-xl shadow-hoverColor sm:w-[90%]">
